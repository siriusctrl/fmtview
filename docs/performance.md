...
the terminal draw byte count includes true-color styling.

Metrics:

- `viewport render CPU` measures repeated wrapped viewport rendering before the
  terminal backend writes anything.
- `terminal draw bytes` measures repeated `ratatui` drawing into a counting
  `CrosstermBackend` writer, including terminal bytes and background-cell count.
