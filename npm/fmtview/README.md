...
`.jsonl`, `.html`, `.toml`, `.txt`, and `.j2` are hints, and unknown extensions are
sniffed from a bounded content prefix where possible.

JSON, JSONL/NDJSON, and XML-compatible markup are formatted. TOML, plain text,
and Jinja templates are passthrough types: they are indexed and previewed
