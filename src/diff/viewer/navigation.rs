...
pub(in crate::diff::viewer) enum DiffJump {
    Next,
    Previous,
}

pub(in crate::diff::viewer) fn jump_change(
    model: &DiffModel,
...
pub(in crate::diff::viewer) fn change_block_starts(changes: &[usize]) -> Vec<usize> {
    changes
        .iter()
        .copied()
        .enumerate()
...
pub(in crate::diff::viewer) fn scroll_by(
    state: &mut DiffViewState,
    model: &DiffModel,
...
pub(in crate::diff::viewer) fn clamp_top(
    state: &mut DiffViewState,
    model: &DiffModel,
    width: usize,
) {
...
pub(in crate::diff::viewer) fn diff_scroll_hint(
