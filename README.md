...
- Highlight common chat roles in JSON and JSONL (`system`, `user`, and
  `assistant`) and prefer those message objects for structure jumps.
...
with `system`, `user`, and `assistant` roles for testing role highlighting and
...
inside another field. Visibility still matters for detail blocks: small fully
visible JSON object/array fields are skipped, while larger composite fields,
partially wrapped blocks, and horizontally clipped blocks remain jump targets.
XML/HTML jumps to start tags when the tag spans more than the current line or
