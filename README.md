...
useful for testing smart block jumps around partially observed inline content.
...
To jump between smart blocks, press `]` or `[`. The viewer skips blocks that
are already fully visible in the current viewport and lands on the next block
that is off-screen, partially wrapped, horizontally clipped in nowrap mode, or
otherwise not fully observed yet. JSON and JSONL previews jump through
composite JSON points such as object/array starts and fields whose value is an
object or array, so expanded JSONL records navigate inside the current record
before falling through to later records. XML/HTML jumps to start tags, Markdown
to headings, TOML to table headers, Jinja to block-level template constructs,
and plain text to paragraph starts.
