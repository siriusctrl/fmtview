...
JSONL record starts, JSON array item objects, Markdown headings, TOML tables,
Jinja blocks, and plain-text paragraph starts can be selected even when they are
already visible, so repeated jumps move through the document's outline instead
of only paging to unseen content. Visibility still matters for detail blocks:
small fully visible JSON object/array fields are skipped, while larger
composite fields, partially wrapped blocks, and horizontally clipped blocks
remain jump targets. XML/HTML jumps to start tags when the tag spans more than
the current line or is not fully observed.
